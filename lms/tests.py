...
class LessonViewSetAPITest(APITestCase):
    def setUp(self):
        ...
        self.course = Course.objects.create(owner=self.user, description='Test Course')
        self.lesson = Lesson.objects.create(course=self.course, owner=self.user, title='Test Lesson')
    ...
    def test_user_can_view_only_own_lessons(self):
        ...
        Lesson.objects.create(course=self.course, owner=other_user, title='Other Lesson')
        self.client.force_authenticate(user=self.user)
        response = self.client.get('/lessons/')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 1)
...
class LessonDetailViewTest(APITestCase):
    ...
    def test_update_lesson(self):
        self.client.force_authenticate(user=self.user)
        response = self.client.patch(f'/lessons/{self.lesson.id}/', {'title': 'Updated Lesson'})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
